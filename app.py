...
st.markdown("""
    ...
    .stock-id { font-size: 1.2rem !important; color: #555 !important; font-weight: normal !important; }
    ...
    .stats-grid { display: flex; justify-content: space-between; margin-top: 15px; border-top: 1px solid #eee; padding-top: 10px;}
    .stat-box { text-align: right; }
    .stat-label { color: #666 !important; font-size: 0.9rem !important; }
    ...
    .stTextInput label { color: #ffffff !important; text-shadow: 1px 1px 2px black; font-weight: bold; }
    ...
    .kd-title { font-size: 1.4rem !important; }
    .kd-val { font-size: 2rem !important; }
    ...
    .stRadio div[role="radiogroup"] > label {
        ...
        border-radius: 8px; margin: 0; border: none; cursor: pointer;
    }
...
def resolve_ticker(user_input):
    ...
    if user_input.isdigit():
        ...
        try:
            ...
            if not hist.empty: return ticker_tw, s.info.get('longName', ticker_tw)
        except: pass
        ...
        try:
            ...
            hist = s.history(period="1d")
            if not hist.empty: return ticker_two, s.info.get('longName', ticker_two)
        except: pass
        return None, None
    else:
        # 美股
...
def generate_narrative_report(name, ticker, latest, inst_df, df, info):
    ...
    if price > ma5 and ma5 > ma10 and ma10 > ma20:
        ...
        tech_trend = "空頭排列，短線趨勢偏弱，上方壓力重重。"
    elif price > ma20:
        tech_trend = "股價站穩月線之上，中期趨勢偏多。"
    else:
        tech_trend = "股價跌破月線，短線轉弱，需觀察支撐。"

    kd_status = "黃金交叉" if k > d else "死亡交叉"
    kd_desc = f"KD ({k:.1f}/{d:.1f}) 呈現 {kd_status}。"
    ...
    if inst_df is not None and not inst_df.empty:
        ...
        inst_desc = f"單日合計 {'買超' if total>0 else '賣超'} {abs(total):,} 張。"
    ...
    <div class="content-card">
        <h3>📊 {name} ({ticker}) 綜合分析</h3>
        <h4>1. 技術指標</h4>
        <p>{tech_trend} {kd_desc}</p>
        <h4>2. 籌碼動向</h4>
...
if target_input:
    with st.spinner("搜尋中..."):
        res_ticker, res_name = resolve_ticker(target_input)
        if res_ticker: target = res_ticker
...
try:
    ...
    with tab1:
        ...
        for ma, c in [('MA5','#1f77b4'), ('MA10','#9467bd'), ('MA20','#ff7f0e'), ('MA60','#bcbd22'), ('MA120','#8c564b'), ('MA240','#7f7f7f')]:
        ...
        fig.update_layout(
            template="plotly_white", height=700, margin=dict(l=10, r=10, t=10, b=10), legend=dict(orientation="h", y=1.01, x=0),
        ...
        st.markdown(f"""<div class="kd-card" style="border-left: 6px solid {kd_color};"><div class="kd-title">KD 指標 (9,3,3)</div><div style="text-align:right;"><div class="kd-val">{latest['K']:.1f} / {latest['D']:.1f}</div></div></div>""", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        for news in news_list:
            st.markdown(f"<div class='content-card news-item'><a href='{news['link']}' target='_blank'>{news['title']}</a><div class='news-meta'>{news['pubDate']}</div></div>", unsafe_allow_html=True)

except Exception as e:
    st.error(f"無法取得資料，請確認代號 ({e})")
