...
st.markdown("""
    ...
    /* 全局背景與字體 */
    .stApp {
        background-color: #0e1117;import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
...
def get_institutional_data(ticker):
    ...
    """
    # 如果 FinMind 沒安裝成功，直接回傳 None，避免當機
...
if latest is not None:
    ...
    if inst_data:
        ...
        if not FINMIND_AVAILABLE:
             reasons.append("⚠️ 法人資料模組未載入 (系統維護中)")
        else:
    ...
    with tab2:
        if inst_data:
            ...
            if ".TW" in ticker_input:
                if not FINMIND_AVAILABLE:
                     st.warning("⚠️ 系統偵測到 FinMind 模組載入失敗，法人籌碼功能暫時停用。")
                else:
