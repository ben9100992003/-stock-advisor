...
st.markdown("""
    ...
    .stock-title { font-size: 1.5rem !important; font-weight: 900 !important; margin: 0; }
    .stock-id { font-size: 1.1rem !important; color: #666 !important; font-weight: normal; }
    
    .price-container {
        display: flex; align-items: baseline; gap: 15px; margin-bottom: 15px;
    }
    ...
    .stRadio div[role="radiogroup"] > label p {
        color: #555 !important; font-weight: bold; font-size: 1rem;
    }
    .stRadio div[role="radiogroup"] > label[data-checked="true"] {
        background-color: #333 !important; /* 選中變深色 */
    }
...
def generate_narrative_report(name, ticker, latest, inst_df, df, info):
    ...
    if inst_df is not None and not inst_df.empty:
        ...
        <tr>
            <td>{last['Date']}</td>
            <td style="color:{'#ef5350' if f_val>0 else '#26a69a'}">{f_val:,}</td>
            <td style="color:{'#ef5350' if t_val>0 else '#26a69a'}">{t_val:,}</td>
            <td style="color:{'#ef5350' if d_val>0 else '#26a69a'}">{d_val:,}</td>
            <td style="color:{'#ef5350' if total>0 else '#26a69a'}"><b>{total:,}</b></td>
        </tr>
        """
    ...
    theme_text = f"<b>{name}</b> 屬於 {sector} 產業。"
...
def analyze_market_index(ticker_symbol):
    try:
        ...
        if price > ma20:
            ...
            color = "#26a69a" if k < d else "#ffeb3b"
