...
def generate_narrative_report(name, ticker, latest, inst_df, df, info):
    ...
    if price > ma5 and ma5 > ma10 and ma10 > ma20:
        tech_trend = "多頭排列"
        tech_desc = "股價沿 5 日線攻擊，趨勢強勁。"
    elif price < ma5 and ma5 < ma10 and ma10 < ma20:
        tech_trend = "空頭排列"
        tech_desc = "上方壓力重重，趨勢偏弱。"
    elif price > ma20:
        tech_trend = "站上月線"
    ...
    # 籌碼面
    inst_html = "<tr><td colspan='4'>暫無資料</td></tr>"
    inst_desc = "暫無數據"
    ...
    summary = info.get('longBusinessSummary', '暫無說明')[:100] + "..."
    
    # 建議
    support = ma10 if price > ma10 else ma20
    resistance = ma5 if price < ma5 else price * 1.05
    action = "觀望"
    if price > ma20 and k > d: action = "偏多操作"
    elif price < ma20 and k < d: action = "保守觀望"
    else: action = "區間震盪"
    ...
    <div class="content-card">
        ...
        <h4>1. 技術指標分析</h4>
        <p><b>趨勢：</b>{tech_trend}。{tech_desc}</p>
        <p><b>KD指標：</b>({k:.1f}/{d:.1f}) 呈現 <b>{kd_status}</b>。</p>
        <h4>2. 三大法人籌碼分析</h4>
        ...
        <p>{inst_desc}</p>
        <h4>3. 公司題材</h4>
        <p><b>產業：</b>{sector}。{summary}</p>
        <h4>4. 💡 操作建議 ({action})</h4>
        <ul>
            <li><b>🟢 進場參考：</b>{support:.2f} 附近</li>
            <li><b>🔴 出場參考：</b>{resistance:.2f} 附近</li>
        </ul>
    </div>
    """
...
def analyze_market_index(ticker_symbol):
    try:
        ...
        comment = f"KD({k:.1f}/{d:.1f})。氣氛：{status}。"
...
if target_input:
    with st.spinner("搜尋中..."):
        ...
        if resolved_ticker:
            ...
            st.error("❌ 找不到該代號，請確認是否正確。")
            target = None

# 大盤
with st.expander("🌍 今日大盤情緒", expanded=False):
    t1, t2 = st.tabs(["🇹🇼 台股", "🇺🇸 美股"])
    with t1:
