...
st.markdown("""
    <style>
    /* 全局字體設定 (強制深色) */
    .stApp { color: #222; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
    ...
    .quote-card, .content-card, .kd-card, .market-summary-box {
        ...
        border-radius: 16px;
        padding: 20px;
        box-shadow: 0 4px 20px rgba(0,0,0,0.25); /* 陰影加深 */
        margin-bottom: 20px;
    ...
    /* --- 1. 報價卡片 (仿 Yahoo App 排版) --- */
    .stock-title { font-size: 1.5rem !important; font-weight: 900 !important; margin-bottom: 5px; line-height: 1.2; }
    .stock-id { font-size: 1rem !important; color: #666 !important; font-weight: normal; margin-left: 5px; }
    
    .price-container {
        display: flex; align-items: baseline; gap: 12px; margin-bottom: 15px;
    }
    .price-big { font-size: 3.8rem !important; font-weight: 800 !important; line-height: 1; letter-spacing: -1px; }
    ...
    .stat-label { font-size: 0.95rem !important; color: #666 !important; }
    .stat-val { font-weight: 700 !important; color: #000 !important; font-size: 1.05rem !important; }
    ...
    .kd-title { font-size: 1.2rem !important; font-weight: bold !important; }
    ...
    .stTabs [data-baseweb="tab-list"] button [data-testid="stMarkdownContainer"] p {
        color: #666 !important; font-weight: 700; font-size: 1rem; text-shadow: none !important;
    }
...
def generate_narrative_report(name, ticker, latest, inst_df, df, info):
    ...
    if price > ma5 and ma5 > ma10 and ma10 > ma20:
        tech_trend = "多頭排列 (MA5>MA10>MA20)"
        tech_desc = "均線向上發散，股價沿 5 日線強勢上攻。"
    elif price < ma5 and ma5 < ma10 and ma10 < ma20:
        tech_trend = "空頭排列 (MA5<MA10<MA20)"
        tech_desc = "均線向下發散，上方壓力重重。"
    elif price > ma20:
        tech_trend = "站上月線 (偏多)"
        tech_desc = "股價站穩月線，中期趨勢偏多。"
    else:
        tech_trend = "跌破月線 (偏空)"
        tech_desc = "股價跌破月線，短線轉弱。"
